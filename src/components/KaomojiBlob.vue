...
<template>
    <a class="before:h-16 before:-mt-16 before:invisible before:block
        ...
        max-w-[95%] min-h-[8.5em] w-fit h-fit bg-gray-900 rounded-2xl font-NottoSansJP text-slate-100
        border-2 hover:border-teal-800 focus:border-teal-800 border-transparent"
       v-on:click="copy()">
        <span class="min-h-[2em] max-h-fit max-w-full mb-3 mt-6 mx-auto px-4 text-sm
     md:text-lg xl:text-xl font-NottoSansJP whitespace-nowrap text-clip overflow-auto">
            {{ kaomoji.string }}
        </span>
